...
NOTE: Version 6.0 (and later) of this plugin will only work with JDK 8, Gradle 5.X and later. Furthermore Subversion will be not longer supported.
...
The information of the used SCM is identified by special directories in the project directory (`'.git'`).
The plugin supports the following SCMs:

* git
* file (limited functionality)

Branches and tags are used for the calculation of the project version. +
...
It is possible to enable this configuration also over the environment variable `'CONTINUOUSRELEASE'` or the project variable `'continuousRelease'`.
